...
- **Cấu hình lưu trữ Cache:** Dùng action `actions/cache` ngay trước bước `npm install`, chỉ định thư mục `node_modules` và key là mã băm vừa tạo:
  ```yaml
...
- **Job 2 (Deploy Fast)**: Máy ảo EC2 kéo trực tiếp Image đã được đóng gói sẵn từ Docker Hub về dùng.
...
Cách cấu hình OIDC:
- **Đăng ký Identity Provider:** Vào AWS IAM, tạo một Identity Provider (Đối tác tin cậy) trỏ URL về kho quản lý token của GitHub (`token.actions.githubusercontent.com`).
- **Tạo IAM Role:** Cấu hình Trust relationships để AWS chỉ chấp nhận token phát ra từ đúng tên Repository và nhánh `main` của dự án.
  *(Ảnh minh họa: Cấu hình Trust Relationship giới hạn quyền truy cập)*
